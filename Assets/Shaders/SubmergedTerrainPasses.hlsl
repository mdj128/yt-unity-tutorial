...
inline half ComputeSubmergedMask(float3 positionWS)
{
    float depth = Vector1_3E4E7F58 - positionWS.y;
    float fade = max(Vector1_5443DC82, 0.0001);
    return saturate(depth * fade);
}

inline half3 ApplySubmergedOverlay(half3 baseColor, float3 positionWS)
{
    half mask = ComputeSubmergedMask(positionWS);
    if (mask <= 0.0001h)
        return baseColor;

    float2 baseUV = positionWS.xz * Vector4_47405F4E.xy + Vector4_47405F4E.zw;
    float time = _TimeParameters.x;
    float size1 = max(Vector1_810FF6ED, 0.001);
    float size2 = max(Vector1_DE7903F8, 0.001);
    float2 speedA = Vector4_85FB229A.xy;
    float2 speedB = Vector4_85FB229A.zw;

    float2 distortionSample = SAMPLE_TEXTURE2D(Texture2D_D71B5CBC, sampler_Texture2D_D71B5CBC, baseUV * 0.25 + time * speedA).rg - 0.5;
    float2 distortion = distortionSample * (Vector1_E79FBA7D * 0.1);

    float2 uv1 = baseUV * size1 + time * speedA + distortion;
    float2 uv2 = baseUV * size2 + time * speedB - distortion;

    float c1 = SAMPLE_TEXTURE2D(Texture2D_D71B5CBC, sampler_Texture2D_D71B5CBC, uv1).r;
    float c2 = SAMPLE_TEXTURE2D(Texture2D_D71B5CBC, sampler_Texture2D_D71B5CBC, uv2).r;

    float blend = saturate(Vector1_A8CC0681);
    float ripplePower = max(Vector1_994EA204, 0.0001);
    float caustic = lerp(c1, pow(saturate(c1 + c2), ripplePower), blend);

    half3 tinted = lerp(baseColor, baseColor * (half3)Color_DB714CC1.rgb, mask);
    half3 caustics = (half3)Color_2B7C42F7.rgb * (half)caustic * mask;
    return saturate(tinted + caustics);
}
...
#endif
{
    ...
    color.a = alpha;
    color.rgb = ApplySubmergedOverlay(color.rgb, inputData.positionWS);
